...
import { ArrowLeft, ChefHat, Clock, Users, UtensilsCrossed } from 'lucide-react'
import type { ReactNode } from 'react'
import { useMemo, useState } from 'react'
import { SidebarLayout } from '../layouts/sidebar-layout'
import { RecipeCookingMode } from '../modules/recipes/recipe-cooking-mode'
import { RecipeHeroBanner } from '../modules/recipes/recipe-hero-banner'
import { StepTimer } from '../modules/recipes/step-timer'
...
export function RecipePage({ id }: Props) {
  ...
  const sortedInstructions = useMemo(
    () => (recipe ? [...recipe.instructions].sort((a, b) => a.step_number - b.step_number) : []),
    [recipe],
  )
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            <div className="flex flex-wrap gap-2 sm:gap-3">
              ...
              <MetaChip
                icon={<Clock className="size-4" />}
                label="Cook"
                value={formatMinutes(recipe.cook_time)}
              />
              <MetaChip
                icon={<Clock className="size-4" />}
                label="Total"
              ...
              <span className="inline-flex items-center gap-2 rounded-lg border border-border/80 bg-card px-3 py-2 text-sm">
                ...
                <span className="font-medium">{capitalizeDifficulty(recipe.difficulty)}</span>
              </span>
            </div>
            <Button
              type="button"
